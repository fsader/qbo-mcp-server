...
/**
 * Register every QBO tool onto the given MCP server, applying the env-driven
 * gating in src/helpers/tool-mode.ts (default: read-only).
 *
 * This is the single source of truth shared by both entrypoints — the stdio
 * server (src/index.ts) and the remote HTTP server (src/http.ts) — so the two
 * can never drift in which tools they expose.
 */
export function registerAllTools(server: McpServer): RegisterAllToolsResult {
  const config = loadToolModeConfig();
  ...
  return { registered, skipped, config };
}

/**
 * Emit the startup tool-gating summary to stderr. stdout is reserved for the
 * stdio MCP transport, so this must never write to stdout.
 */
export function logRegistrationSummary(result: RegisterAllToolsResult): void {
  const { registered, skipped, config } = result;
  console.error(
    `[qbo-mcp] QBO_TOOL_MODE=${config.mode} | ` +
      `delete tools enabled: ${config.enableDeleteTools} | ` +
      `registered: ${registered.length} | skipped: ${skipped.length}`
  );
  if (config.allowedTools.size > 0) {
    console.error(
      `[qbo-mcp] QBO_ALLOWED_TOOLS active (${config.allowedTools.size} names) — ` +
  ...
  if (["1", "true", "yes"].includes((process.env.QBO_DEBUG_TOOLS ?? "").toLowerCase())) {
    console.error(
      `[qbo-mcp] registered tools (${registered.length}): ${[...registered].sort().join(", ")}`
    );
    console.error(
      `[qbo-mcp] skipped tools (${skipped.length}): ${[...skipped].sort().join(", ")}`
    );
  }
}
